...
#c("num_unit", "region_neighbor", "region_zip", "region_city", "zoning_landuse_county")
...
for (big_factor_name in big_factor_names){
  ...
  t = clean_prop[, .(count=.N), by=big_factor_name]
  t = t[order(-t$count)]
  t = t[count>300]
  if (nrow(t)>1000)
    t = t[1:1000]
  
  clean_prop[[paste(big_factor_name, "fac", sep="_")]] = as.factor(ifelse(clean_prop[[big_factor_name]] %in% t[[big_factor_name]], clean_prop[[big_factor_name]], 0))
}
