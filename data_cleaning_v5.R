...
##
clust_df = data.frame(latitude = clean$latitude, longitude=clean$longitude)#, logerror=clean$logerror)
km_model = kcca(clust_df, k=50, kccaFamily("kmeans"))
clean$long_lat_cluster = as.factor(predict(km_model, clust_df))
...
clean$num_garage_fac=as.factor(clean$num_garage)
clean$num_garage_1_fac=as.factor(clean$num_garage_1)
clean$num_pool_fac=as.factor(clean$num_pool)
clean$num_pool_1_fac=as.factor(clean$num_pool_1)
clean$num_room_fac=as.factor(clean$num_room)
clean$num_story_fac=as.factor(clean$num_story)
clean$num_unit_fac=as.factor(clean$num_unit)
clean$num_bathroom_fac=as.factor(clean$num_bathroom)
clean$num_bedroom_fac=as.factor(clean$num_bedroom)
clean$num_bath_fac=as.factor(clean$num_bath)
clean$num_75_bath_fac=as.factor(clean$num_75_bath)
clean$num_fireplace_fac=as.factor(clean$num_fireplace)
...
t = select(clean, zoning_landuse_county)
...
t = select(clean, zoning_property)
...
########################
# add top 50 of each long factor as factor

t = clean[, .(count=length(id_parcel)), by=region_neighbor]
t = t[order(-t$count)][1:50,]
clean$region_neigbor_fac = as.factor(ifelse(clean$region_neighbor %in% t$region_neighbor, clean$region_neighbor, 0))

t = clean[, .(count=length(id_parcel)), by=region_zip]
t = t[order(-t$count)][1:50,]
clean$region_zip_fac = as.factor(ifelse(clean$region_zip %in% t$region_zip, clean$region_zip, 0))

t = clean[, .(count=length(id_parcel)), by=zoning_landuse_county]
t = t[order(-t$count)][1:50,]
clean$zoning_landuse_county_fac = as.factor(ifelse(clean$zoning_landuse_county %in% t$zoning_landuse_county, clean$zoning_landuse_county, 0))

t = clean[, .(count=length(id_parcel)), by=zoning_landuse]
t = t[order(-t$count)][1:50,]
clean$zoning_landuse_fac = as.factor(ifelse(clean$zoning_landuse %in% t$zoning_landuse, clean$zoning_landuse, 0))

t = clean[, .(count=length(id_parcel)), by=zoning_property]
t = t[order(-t$count)][1:50,]
clean$zoning_property_fac = as.factor(ifelse(clean$zoning_property %in% t$zoning_property, clean$zoning_property, 0))
